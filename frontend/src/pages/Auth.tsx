...
/* ---------------------------------------------------------
   VALIDATION DATA
   Keep these lists small and easy to extend later. For real
   protection (esp. profanity + disposable emails) you should
   also validate on the backend — this is just the UX layer.
--------------------------------------------------------- */

// Common disposable/temp-mail domains. Add more as needed.
...
// A small set of blocked words for name/email screening.
...
// Basic RFC-ish email shape check (good enough for frontend gating)
...
// Checks a string against the blocked-word list (case-insensitive, substring match)
function containsBlockedWord(value: string): boolean {
  const lower = value.toLowerCase();
  return BLOCKED_WORDS.some((word) => lower.includes(word));
}

// Full email validation: format + not disposable
...
// Full name validation: not empty, no blocked words, reasonable length
...
export default function Auth() {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    // ---- USER PATH: run all frontend validations before hitting the backend ----
    if (role === "user") {
    ...
    try {
      ...
      // ---- DUPLICATE ACCOUNT CHECK (signup only) ----
      // Prevents creating a second account with an email or phone
      // that's already registered. This is a frontend UX guard —
      // your backend should also enforce a unique constraint on
      // email/phone so this can't be bypassed via direct API calls.
      if (role === "user" && authMode === "signup") {
        ...
        if (check.exists) {
          if (check.emailExists) {
            ...
            setError(
              `An account with mobile number '+91 ${cleanPhone}' already exists. Please sign in instead.`
            );
          }
          setLoading(false);
          return;
      ...
      // ---- SIGN-IN PATH: verify the account actually exists ----
      // If someone tries to "sign in" with an email that has no
      // account, tell them to create one instead of silently
      // creating a session for a non-existent user.
      if (role === "user" && authMode === "signin") {
        try {
          ...
          // If the existence check itself fails (network issue etc.),
          // fall through and let the normal sign-in flow proceed.
        }
      }
  ...
  return (
    // Reduced vertical padding (py-8 -> py-6) so the whole page feels tighter
    <main className="relative min-h-screen w-full overflow-hidden bg-[#F7F4EE] font-sans text-[#2A2421] flex items-center justify-center py-6 px-4 sm:px-6 select-none">
      ...
      {/* Card container — reduced max width (4xl -> 3xl) to shrink overall footprint */}
      <div className="relative z-10 w-full sm:max-w-100 lg:max-w-3xl rsf-card-in">
        <div className="relative overflow-hidden rounded-4xl border border-white/90 bg-white/70 shadow-[0_30px_70px_rgba(42,36,33,0.12)] backdrop-blur-[32px] lg:grid lg:grid-cols-5">
          ...
          {/* ============ LEFT: brand panel ============ */}
          {/* Reduced padding (p-10 -> p-8) and tightened internal spacing */}
          <div className="hidden lg:flex lg:col-span-2 relative flex-col justify-between overflow-hidden bg-linear-to-br from-[#8E3D51] via-[#8E3D51] to-[#722F40] p-8 text-[#F7F4EE]">
            ...
            {/* Everything centered as a column instead of the old off-center ml offsets */}
            <div className="relative z-10 flex flex-col items-center text-center">
              {/* Logo badge: cream circle behind the (dark) logo so it never blends
                  into the maroon panel. Also wrapped in a Link back to home. */}
              <Link
                to="/"
          ...
          {/* Reduced padding (p-7/9/10 -> p-6/7/8) for a more compact card */}
          <div className="lg:col-span-3 p-6 sm:p-7 lg:p-8">

            {/* Compact brand header — mobile & tablet only */}
            <div className="lg:hidden mb-6 text-center">
              <div className="inline-flex flex-col items-center">
                {/* Same cream-badge treatment here so the logo stays visible on mobile too */}
                <Link
                  to="/"
            ...
            {/* Form — reduced vertical gap between fields (space-y-4 -> space-y-3) */}
            <form onSubmit={handleAuthSubmit} className="space-y-3">
              ...
              <div>
                ...
                {/* Small hint so users know why some emails get rejected */}
                {role === "user" && authMode === "signup" && (
