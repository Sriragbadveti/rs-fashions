...
import { FiArrowUpRight, FiInstagram, FiMail, FiHeart } from "react-icons/fi";
  ...
  const quickConnectActions = [
    {
      ...
      icon: <FaWhatsapp size={18} className="text-emerald-600" />,
      border: "border-emerald-200", bg: "from-[#F0FDF4] to-[#DCFCE7]", hoverBorder: "hover:border-emerald-400",
    },
    {
      label: "Email", href: "mailto:support@rsfashions.com", external: false,
      icon: <FiMail size={18} className="text-[#8E3D51]" />,
      border: "border-rose-200", bg: "from-[#FFF5F6] to-[#FFE2E7]", hoverBorder: "hover:border-rose-400",
    },
    {
      label: "Instagram", href: "https://instagram.com", external: true,
      icon: <FiInstagram size={18} className="text-[#BE185D]" />,
      border: "border-pink-200", bg: "from-[#FDF2F8] to-[#FCE7F3]", hoverBorder: "hover:border-pink-400",
    },
  ];
          ...
          <div className="lg:col-span-5">
            <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-white/10 p-6 shadow-[0_16px_40px_-8px_rgba(0,0,0,0.2)] backdrop-blur-xl sm:p-8">
              ...
              <div className="relative mt-6 grid grid-cols-3 gap-3">
                {quickConnectActions.map((action) => (
                  <a
                    key={action.label} href={action.href}
                    target={action.external ? "_blank" : undefined}
                    rel={action.external ? "noreferrer" : undefined}
                    aria-label={`${action.label} Support`}
                    className={`group/btn flex flex-col items-center justify-center gap-1.5 rounded-2xl border ${action.border} bg-linear-to-b ${action.bg} p-3.5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${action.hoverBorder}`}
                  >
                    <span className="transition-transform duration-300 group-hover/btn:scale-110">{action.icon}</span>
                    <span className="text-[10px] font-bold uppercase tracking-wide text-stone-800">{action.label}</span>
                  </a>
                ))}
              </div>
            </div>
